...
- Workspace status generation now scans each active-task pair once, avoids
  temporary task slices, reuses the loaded agent list, and batches endpoint
  reads instead of querying once per agent.
